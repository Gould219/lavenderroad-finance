...
contract Lroad is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the LROAD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Lavender Road", "LROAD") {
        // Mints 1 LROAD to contract creator for initial pool setup
    ...
    function _getLroadPrice() internal view returns (uint256 _lroadPrice) {
        try IOracle(lroadOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Lroad: failed to fetch LROAD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _lroadPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_lroadPrice >= taxTiersTwaps[tierId]) {
    ...
    function setLroadOracle(address _lroadOracle) public onlyOperatorOrTaxOffice {
        require(_lroadOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints LROAD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of LROAD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentLroadPrice = _getLroadPrice();
            currentTaxRate = _updateTaxRate(currentLroadPrice);
            if (currentLroadPrice < burnThreshold) {
                burnTax = true;
            }
        }
