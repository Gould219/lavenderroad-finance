...
/**
 * An API module of Lavender Road contracts.
...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    // Uniswap V2 Pair
    this.LROADWFTM_LP = new Contract(externalTokens['LROAD-FTM-LP'][0], IUniswapV2PairABI, provider);
  ...
  unlockWallet(provider: any, account: string) {
    ...
    this.LROADWFTM_LP = this.LROADWFTM_LP.connect(this.signer);
  ...
  async getTombStat(): Promise<TokenStat> {
    const { LroadFtmRewardPool } = this.contracts;
    const supply = await this.LROAD.totalSupply();
    const tombRewardPoolSupply = await this.LROAD.balanceOf(LroadFtmRewardPool.address);
    // const tombRewardPoolSupply2 = await this.LROAD.balanceOf(LroadFtmLPLroadRewardPool.address);
    // const tombRewardPoolSupplyOld = await this.LROAD.balanceOf(LroadFtmLPLroadRewardPoolOld.address);
    const tombCirculatingSupply = supply.sub(tombRewardPoolSupply);
      // .sub(tombRewardPoolSupply2);
      // .sub(tombRewardPoolSupplyOld);
    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.LROAD);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.LROAD.decimal, 0),
      circulatingSupply: getDisplayBalance(tombCirculatingSupply, this.LROAD.decimal, 0),
    };
  }
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    const supply = await this.LBURGER.displayedTotalSupply();
    return {
  ...
  async getTombStatInEstimatedTWAP(): Promise<TokenStat> {
    const { SeigniorageOracle, LroadFtmRewardPool } = this.contracts;
    const expectedPrice = await SeigniorageOracle.twap(this.LROAD.address, ethers.utils.parseEther('1'));
  ...
  async getLroadPriceInLastTWAP(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getLroadUpdatedPrice();
  }

  async getBondsPurchasable(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getBurnableLroadLeft();
  }
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'LROAD' ? await this.getTombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'LROAD') {
      ...
      if (Date.now() - startDateTime.getTime() > FOUR_DAYS) {
        return await poolContract.epochLroadPerSecond(1);
      }
    ...
    if (depositTokenName.startsWith('LROAD')) {
      ...
      return rewardPerSecond.mul(4000).div(10000);
    }
  }
  ...
  async buyBonds(amount: string | number): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const treasuryTombPrice = await Treasury.getLroadPrice();
  ...
  async redeemBonds(amount: string): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const priceForTomb = await Treasury.getLroadPrice();
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'LROAD') {
        return await pool.pendingLROAD(poolId, account);
      } else {
  ...
  //   if (!ready) return;
  //   const { chainId } = this.config;
  //   console.log('log->chainId', this.config);
  ...
  async getGenesisPoolStartAndEndTime(): Promise<AllocationTime> {
    const { LroadFtmRewardPool } = this.contracts;
    const startTimestamp: BigNumber = await LroadFtmRewardPool.poolStartTime();
  ...
  async quoteFromSpooky(tokenAmount: string, tokenName: string): Promise<string> {
    ...
    const { _reserve0, _reserve1 } = await this.LROADWFTM_LP.getReserves();
    let quote;
    if (tokenName === 'LROAD') {
  ...
  async getTShareSwapperStat(address: string): Promise<TShareSwapperStat> {
    ...
    const rateTSharePerTombBN = await TShareSwapper.getMeteprAmountPerLroad();
